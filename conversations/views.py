...
def go_conversation(request, a_pk, b_pk):
    ...
    if user_one is not None and user_two is not None:
        try:
            conversation = models.Conversations.objects.get(Q(participants=user_one) & Q(participants=user_two))
...
class ConversationDetailView(View):
    def get(self, *args, **kwargs):
        ...
        if not conversation:
            raise Http404()
        return render(self.request, "conversations/conversation_detail.html", {"conversations": conversation})
